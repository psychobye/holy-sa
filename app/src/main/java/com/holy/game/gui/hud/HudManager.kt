...
import com.holy.game.databinding.HudBinding
import com.holy.game.gui.util.ConvertViewCoordsToGta
...
import com.holy.game.gui.util.Utils.getBitmapFromImageView
...
class HudManager : com.holy.game.gui.hud.Chat() {
    ...
    private external fun nativeSetRadarBg(buffer: java.nio.ByteBuffer, width: Int, height: Int)
    ...
    fun updateBars(health: Int, armour: Int, hunger: Int) {
        activity.runOnUiThread {
            ...
            if (health >= 100) {
                binding.hudHpLayout.visibility = View.GONE
            } else {
                binding.hudHpLayout.visibility = View.VISIBLE
            }
    ...
    fun initServerLogo() {
        activity.runOnUiThread {
            val serverColor = com.holy.launcher.storage.Storage.getProperty(com.holy.launcher.domain.enums.StorageElements.SERVER_COLOR, activity)
            val serverName = com.holy.launcher.storage.Storage.getProperty(com.holy.launcher.domain.enums.StorageElements.SERVER_NAME, activity)
            val serverMulti = com.holy.launcher.storage.Storage.getProperty(com.holy.launcher.domain.enums.StorageElements.SERVER_MULTI, activity)
    ...
    init {
        ...
        binding.healthProgress.progress     = 70
        ...
        binding.btnChat.setOnClickListener {
            activity.runOnUiThread {
                val chatBox = activity.findViewById<ConstraintLayout>(R.id.chat_box)
                if (chatBox != null && chatBox.visibility == View.VISIBLE) {
                    hideChat()
                    binding.btnChat.setImageResource(R.drawable.ic_chat_off)
                } else {
                    showChat()
        ...
        hud_bg.post {
            ...
            val bmp = getBitmapFromImageView(hud_bg)
            val buffer = bitmapToByteBuffer(bmp, Utils.PixelOrder.BGRA)

            nativeSetRadarBg(buffer, bmp.width, bmp.height)
