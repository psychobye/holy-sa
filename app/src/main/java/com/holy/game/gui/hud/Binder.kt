...
import com.holy.game.databinding.BinderBinding
import com.holy.game.gui.NativeGui
import com.holy.game.gui.styling.ColorPicker
import com.holy.game.gui.styling.ColorPickerListener
...
class Binder : NativeGui<BinderBinding>(BinderBinding::class), BinderAdapterListener{
    ...
    init {
        ...
        binding.bindingAddSucces.setOnClickListener { view: View? ->
            ...
            adapter.addItem(com.holy.game.gui.hud.BinderItem(input.text.toString(), -0xff6978))
    ...
    class BinderAdapter(val listener: BinderAdapterListener) : BouncyRecyclerView.Adapter<BinderAdapter.ViewHolder>() {
        ...
        var list: MutableList<com.holy.game.gui.hud.BinderItem> = ArrayList()
        ...
        private fun loadData() {
            try {
                ObjectInputStream(activity.openFileInput(BINDER_FILE_NAME)).use { ois ->
                    list = ois.readObject() as ArrayList<com.holy.game.gui.hud.BinderItem>
                }
            } catch (ex: Exception) {
                println(ex.message)
            }
        ...
        fun addItem(item: com.holy.game.gui.hud.BinderItem) {
            list.add(item)
    ...
    override fun onSelectedItem(text: String) {
        ...
        for (message in messages) {
            try {
                ...
                com.holy.game.gui.hud.Chat.SendChatMessage(message.toByteArray(charset("windows-1251")))
            } catch (e: UnsupportedEncodingException) {
                throw RuntimeException(e)
            }
        }
        com.holy.game.gui.hud.Chat.chat_input.text.clear()
    }
