...
import com.holy.launcher.download.DownloadListener
import com.holy.launcher.download.FileDownloader
import com.holy.launcher.service.ActivityService
import com.holy.launcher.service.impl.ActivityServiceImpl
import com.holy.launcher.ui.adapters.LoaderSliderAdapter
import com.holy.launcher.utils.MainUtils
import com.holy.launcher.utils.MainUtils.Companion.type
...
class LoaderActivity : AppCompatActivity() {
    ...
    private fun installGame(type: com.holy.launcher.domain.enums.DownloadType) {
        when (type) {
            com.holy.launcher.domain.enums.DownloadType.RELOAD_GAME_FILES -> {
            ...
            com.holy.launcher.domain.enums.DownloadType.UPDATE_APK -> {
    ...
    private fun loadResources() {
        val retrofit = Retrofit.Builder()
            .baseUrl(com.holy.launcher.config.Config.LIVE_RUSSIA_RESOURCE_SERVER_URL)
    ...
    fun installApk() {
        try {
            val file = File(getExternalFilesDir(null).toString(), com.holy.launcher.config.Config.APK_FILE_NAME)
            var intent: Intent
            if (file.exists()) {
                run {
                ...
                com.holy.launcher.service.impl.ActivityServiceImpl.showErrorMessage("Ошибка установки: файл не найден", this)
                finish()
                exitProcess(EXIT_SUCCESS_STATUS)
            }
        } catch (e: Exception) {
